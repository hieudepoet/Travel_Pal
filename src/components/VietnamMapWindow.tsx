...
export default function VietnamMapWindow({ className, style }: VietnamMapWindowProps) {
    ...
    const handleRouteProvinceClick = (provinceId: string) => {
    ...
    const handleRouteProvinceHover = (provinceId: string | null) => {
        if (provinceId) {
            const city = citiesByProvince.get(provinceId);
            if (city) {
                setHoveredCity(city);
            ...
            <div className="map-pane map-pane--main relative w-1/3 h-full flex justify-center items-center overflow-hidden">
                {showRoute && customRoute ? (
                    <>
                        <RouteVisualization
                            route={customRoute}
                            onProvinceClick={handleRouteProvinceClick}
                            onProvinceHover={handleRouteProvinceHover}
                        />

                        {/* Popups for Route View */}
                        {hoveredCity && provinceLookup.get(hoveredCity.provinceId) && hoveredCity.id !== selectedCity?.id && (
                            <div
                                className="absolute transform -translate-x-1/2 -translate-y-1/2 pointer-events-none bg-white text-black px-3 py-1 rounded shadow-lg z-20"
                                style={getOverlayStyle(provinceLookup.get(hoveredCity.provinceId)!.centroid, vietnamView)}
                            >
                                <p className="font-bold text-sm">{hoveredCity.name}</p>
                            </div>
                        )}
                        ...
                        {selectedCity && provinceLookup.get(selectedCity.provinceId) && (() => {
                            const regionCenter = {
                                x: vietnamView.minX + vietnamView.width / 2,
                                y: vietnamView.minY + vietnamView.height / 2,
                            };
                            const cityCenter = {
                                x: provinceLookup.get(selectedCity.provinceId)!.centroid[0],
                                y: provinceLookup.get(selectedCity.provinceId)!.centroid[1],
                    ...
                    <>
                        <svg viewBox={vietnamViewBox} className="w-full h-full" style={{ filter: 'drop-shadow(0 0 10px rgba(0,0,0,0.3))' }} preserveAspectRatio="xMidYMin meet">
                            <title>Vietnam map overview</title>
                            {overviewPaths}
                        </svg>
                        {hoveredRegion && regionLookup[hoveredRegion] && (
        ...
        return (
            <div className="map-pane map-pane--main relative w-1/3 h-full flex justify-center items-center overflow-hidden">
                ...
                {showRoute && customRoute ? (
                    ...
                    <>
                        <svg viewBox={regionDef.viewBox} className="w-full h-full" preserveAspectRatio="xMidYMin meet">
                            <title>Zoomed view for {regionDef.name}</title>
                            {detailPaths}
                        </svg>
    ...
    if (!isLoaded) {
        return <div className="w-full h-full flex items-center justify-center bg-[#020612] text-white">Loading Maps...</div>;
    }
...
>>>>>>> e336f066fceb6b7058405cbcf286ce8bfa282b7a
